...
export class Physics extends Discipline {
  ...
  conductLab(group: StudentGroup): void {
    ...
    if (maxSubgroups === 0) {
      throw new Error(
        `Group of ${group.size} students is too small for laboratory work (minimum 10 required)`
      );
    }

    if (maxSubgroups >= 1) {
      this.onNotify(
        `Physics lab conducted for group ${group.name} (${maxSubgroups} subgroup(s) of 10+ students)`
      );
    }
  }
